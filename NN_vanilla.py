...
while loss >= threshold and i < max_iter:
        ...
        z1 = sp.dot(weights1.T, input_data) #8 x 1 linear transformation no.1
        a1 = sp.maximum(0, z1) #8 x 1 #relu no.1
        z2 = sp.dot(weights2.T, a1) #5 x 1  linear transformation no. 2
        a2 = sp.maximum(0, z2) #5 x 1 relu no.2 
        
        loss = 1/len(output_data) * sp.sum((a2 - output_data)**2) #mean squared error
